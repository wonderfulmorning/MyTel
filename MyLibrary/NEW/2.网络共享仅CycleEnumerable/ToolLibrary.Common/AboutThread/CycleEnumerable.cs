...
namespace ToolLibrary.Common.AboutThread
{
    ...
    public class CycleEnumerable
    {
        ...
        /// </summary>
        int isDead = 0;
        public bool IsDead { get { return this.isDead == 1; } }
        ...
        public void Start()
        {
            ...
            if (this.IsDead)
            {
                throw new InvalidOperationException("对象处于死亡状态");
            }
        ...
        void InnerWork()
        {
            ...
            switch (current.HandleType)
            {
                ...
                case CycleEnumerableEnum.CancelCyle:
                    ...
                    break;
                    //终结对象
                case CycleEnumerableEnum.Terminal:
                    Interlocked.CompareExchange(ref isDead, 1, 0);
                    //取消工作
                    this._cannelsource.Cancel();
                    break;
                //直接进行后续操作
            ...
            //if (!hasvalue)
            {
                ...
                //20170331  重新调用迭代器要求此时_cannelsource对象没有被cancel
                if (!this._cannelsource.IsCancellationRequested)
                {
                    //开始进行迭代
                    hasvalue = this.MoveNext();
